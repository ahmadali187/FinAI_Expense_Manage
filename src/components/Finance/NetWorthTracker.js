    ...
    <div className="glass-card" style={{ background: 'rgba(30, 41, 59, 0.75)', border: '1px solid rgba(255, 255, 255, 0.15)', marginTop: '24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', flexWrap: 'wrap', gap: '12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div>
            <h3 style={{ margin: 0, fontSize: '1.15rem', fontWeight: 800, color: '#ffffff' }}>Net Worth & Debt Tracker</h3>
            <span style={{ fontSize: '0.8rem', color: '#94a3b8' }}>Formula: <strong>Net Worth = Assets - Liabilities</strong></span>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '14px', marginBottom: '16px' }}>
        ...
        <div style={{ background: 'rgba(99, 102, 241, 0.12)', border: '1px solid rgba(99, 102, 241, 0.3)', padding: '14px', borderRadius: '12px' }}>
          <span style={{ fontSize: '0.75rem', color: '#94a3b8', display: 'block' }}>Total Net Worth</span>
          <strong style={{ fontSize: '1.3rem', color: net_worth >= 0 ? '#ffffff' : '#ef4444', display: 'block', marginTop: '4px' }}>{formatAmount(net_worth)}</strong>
        </div>
      </div>

      {showAddModal && (
        <form onSubmit={handleAddSubmit} style={{ background: 'rgba(15, 23, 42, 0.8)', padding: '14px', borderRadius: '12px', marginBottom: '16px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '10px' }}>
          <select
