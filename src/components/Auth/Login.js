...
const Login = () => {
  ...
  const { login, loginWithGoogle, lastUserEmail } = useContext(UserContext);
  ...
  return (
    <div className="glass-card" style={{ maxWidth: '440px', margin: '40px auto' }}>
      ...
      {loading && <div style={{ color: 'var(--primary-glow)', textAlign: 'center', marginBottom: '10px' }}>Authenticating with Google...</div>}
      ...
      <p style={{ textAlign: 'center', marginTop: '20px', color: 'var(--text-muted)', fontSize: '0.9rem' }}>
        Don't have an account? <Link to="/register" style={{ color: 'var(--primary-glow)', fontWeight: 600 }}>Register here</Link>
      </p>
    </div>
  );
};
