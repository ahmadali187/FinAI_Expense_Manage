      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px' }}>
          {calculation_details.map((item, idx) => (
            <div key={idx} style={{ background: 'rgba(30, 41, 59, 0.6)', padding: '10px 14px', borderRadius: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '0.85rem', color: '#e2e8f0' }}>{item.label}</span>
              <strong style={{ fontSize: '0.9rem', color: item.sign === '-' ? '#ef4444' : '#10b981' }}>
        ...
        <div style={{ background: 'rgba(16, 185, 129, 0.15)', border: '1px solid rgba(16, 185, 129, 0.3)', padding: '14px', borderRadius: '12px', textAlign: 'center' }}>
          <span style={{ fontSize: '0.75rem', color: '#a7f3d0', textTransform: 'uppercase', fontWeight: 700, display: 'block' }}>Resulting Daily Limit</span>
          <strong style={{ fontSize: '1.8rem', color: '#10b981' }}>{formatAmount(safe_to_spend_today)} / day</strong>
          <span style={{ fontSize: '0.75rem', color: '#cbd5e1', display: 'block', marginTop: '2px' }}>
